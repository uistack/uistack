<script src="<?php echo e(asset('public/admin_assets/vendors/modal/js/classie.js')); ?>"></script>
<script src="<?php echo e(asset('public/admin_assets/vendors/modal/js/modalEffects.js')); ?>"></script>
...
<script type="text/javascript">
    ...
    if ($('#main-image').val() != "") {
//        $.get("<?php echo e(url('/')); ?>/<?php echo e(Lang::getlocale()); ?>/media/"+$('#main-image').val()+"", function(data) {
//          $('#main-image-thumbnail').attr("src", data['url']);
//        });
    }

    // Old gallery images
    oldGalleryImages = '<?php echo e(json_encode(old('gallery_images'))); ?>';
    var oldGallery = JSON.parse(oldGalleryImages.replace(/&quot;/g, '"'));
    if (oldGallery != null) {
        gallery.innerHTML = '';
        for (var i = 0; i < oldGallery.length; i++) {
            $.get("<?php echo e(url('/')); ?>/<?php echo e(Lang::getlocale()); ?>/media/" + oldGallery[i] + "", function(data) {
    ...
    function fetchImageUrlToGallery() {
        ...
        jQuery.ajax({
            url: "<?php echo e(url('/')); ?>/fetch-image-url",
            type: "POST",
            data: {'url': imgUrl},
