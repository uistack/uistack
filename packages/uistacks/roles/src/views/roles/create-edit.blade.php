                            ...
                            @if(isset($permissions) && count($permissions)>0)
                                ...
                                @foreach($permissions as $per)
                                    @if($per->id == 20 || $per->id == 21)
                                        <div class="checkbox">
                                            <label><input name="permission_id[]" checked type="checkbox" value="{{ $per->id }}" class="check_list" @if(isset($arr_role_permission) && count($arr_role_permission) > 0) @if(in_array($per->id,$arr_role_permission)) checked @endif  @endif> {{ $per->name }}</label>
                                        </div>
                                    @else
                                        <div class="checkbox">
                                            <label><input name="permission_id[]" type="checkbox" value="{{ $per->id }}" class="check_list" @if(isset($arr_role_permission) && count($arr_role_permission) > 0) @if(in_array($per->id,$arr_role_permission)) checked @endif  @endif> {{ $per->name }}</label>
                                        </div>
                                    @endif
                                @endforeach
                                {{--<div class="col-md-3">--}}
