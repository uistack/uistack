...
@section('page_title')
    {{ trans('Activities::activities.activities') }}
@endsection
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="admin-top-operation">
                <a class="btn btn-default" href="{{ action('\UiStacks\Activities\Controllers\ActivitiesController@create')}}">{{ trans('Core::operations.create') }} {{ trans('Activities::activities.activity') }}</a>
            </div>
            @if($items->count() || $_GET)
                @include('Activities::activities.filter')
            @endif
            @if($items->count())
                <form method="POST" action="{{ action('\UiStacks\Activities\Controllers\ActivitiesController@bulkOperations')}}" id="bulk" class="form-inline">
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-hover">
                            ...
                            <tr>
                                ...
                                <th>{{ trans('Activities::activities.name') }}</th>
                            ...
                            @foreach($items as $item)
                                <tr @if($item->trans) data-title="{{ $item->trans->name }}" @endif>
                                    ...
                                    @endif
                                    <td>
                                        <a href="{{ action('\UiStacks\Activities\Controllers\ActivitiesController@edit', $item->id) }}"><i class="fa fa-edit"></i> {{ trans('Core::operations.edit') }}</a>
            ...
            @else
                <div class="text-center">
                    @if($_GET)
                        <h1>{{ trans('admin.no_results_found') }} <a href="{{ action('\UiStacks\Activities\Controllers\ActivitiesController@index')}}">{{ trans('admin.back') }}</a></h1>
                    @elseif(Request::is(''.Lang::getlocale().'/admin/activities'))
                        <h1>{{ trans('admin.no_data_added_before') }} <a href="{{ action('\UiStacks\Activities\Controllers\ActivitiesController@create')}}">{{ trans('Core::operations.create') }} {{ trans('Activities::activities.activity') }}</a></h1>
                    @endif
                </div>
            @endif
        </div>
