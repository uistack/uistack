@php
    $breadcrumbs = [
                        ['url' => '', 'name' => trans('Widgets::widgets.ads')]
    ];
    $dbTable = '';
    if($items->count()){
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="admin-top-operation">
                <a class="btn btn-default" href="{{ url('/').'/'.Lang::getlocale().'/admin/widgets/create' }}">{{ trans('Core::operations.create') }} {{ trans('Widgets::widgets.ad') }}</a>
                {{--{{dd('sd')}}--}}
            </div>
            @if($items->count() || $_GET)
            ...
            @endif
            @if($items->count())
                <form method="POST" action="{{ action('\UiStacks\Widgets\Controllers\WidgetController@bulkOperations')}}" id="bulk" class="form-inline">
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-hover">
                            ...
                            <tr>
                                ...
                                <th>{{ trans('Widgets::widgets.name') }}</th>
                            ...
                            @foreach($items as $item)
                                <tr @if($item->trans) data-title="{{ $item->trans->name }}" @endif>
                                    ...
                                    @endif
                                    <td>
                                        <a href="{{ url('/').'/'.Lang::getlocale().'/admin/widgets/'.$item->id.'/edit' }}"><i class="fa fa-edit"></i> {{ trans('Core::operations.edit') }}</a>
            ...
            @else
                <div class="text-center">
                    @if($_GET)
                        <h1>{{ trans('admin.no_results_found') }} <a href="{{ url('/').'/'.Lang::getlocale().'/admin/widgets' }}">{{ trans('admin.back') }}</a></h1>
                    @elseif(Request::is(''.Lang::getlocale().'/admin/widgets'))
                        <h1>{{ trans('admin.no_data_added_before') }} <a href="{{ url('/').'/'.Lang::getlocale().'/admin/widgets/create' }}">{{ trans('Core::operations.create') }} {{ trans('Widgets::widgets.ad') }}</a></h1>
                    @endif
                </div>
            @endif
        </div>
