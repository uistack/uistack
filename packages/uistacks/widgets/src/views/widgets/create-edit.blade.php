@php
    ...
    $breadcrumbs[] =  ['url' => url('/').'/'.Lang::getLocale().'/admin/widgets', 'name' => trans('Widgets::widgets.ads')];
    $action = action('\UiStacks\Widgets\Controllers\WidgetController@store');
    $method = '';
    $backFieldLabel = trans('admin.add_new_after_save');
    $submitButton = trans('admin.save');
    if(Request::is('*/edit')){
        $pageNameMode = trans('Core::operations.edit');
        $breadcrumbs[] =  ['url' => '', 'name' => trans('Core::operations.edit').' '.trans('Widgets::widgets.ad')];
        $action = action('\UiStacks\Widgets\Controllers\WidgetController@update', $item->id);
        $method = 'PATCH';
        $backFieldLabel = trans('admin.back_after_update');
        $submitButton = trans('admin.update');
    }else{
        $breadcrumbs[] =  ['url' => '', 'name' => trans('Core::operations.create').' '.trans('Widgets::widgets.ad')];
    }
@endphp

@extends('admin.master')
@section('page_title')
    {{ trans('Widgets::widgets.ads') }}: {{ $pageNameMode }} {{ trans('Widgets::widgets.ad') }}
@endsection
@section('content')
    <!-- Include Media model -->
        ...
        <div class="col-md-12">
            <div class="panel panel-primary">
                <div class="panel-heading">
                    <h3 class="panel-title"><i class="livicon" data-name="list" data-size="18" data-c="#fff" data-hc="#fff" data-loop="true"></i> {{ $pageNameMode }} {{ trans('Widgets::widgets.ad') }}</h3>
                </div>
                    ...
                    <form action="{{ $action }}" method="POST" role="form" >
                        ...
                        <div class="col-md-9">
                            ...
                            @include('Core::fields.select', [
                                'field_name' => 'position',
                                'name' => trans('Widgets::widgets.position'),
                                'value' => $statusValue,
                                'options' => [
                                                ['value' => '0', 'name' => '-- '.trans('Core::operations.select').' --'],
                                                ['value' => 1, 'name' => trans('Widgets::widgets.home_top_left')],
                                                ['value' => 2, 'name' => trans('Widgets::widgets.home_top_right')],
                                                ['value' => 3, 'name' => trans('Widgets::widgets.home_bottom_left')],
                                                ['value' => 4, 'name' => trans('Widgets::widgets.home_bottom_right')]
                                ]
                            ])

                            <div class="form-group @if ($errors->has('scripts')) has-error @endif">
                                <label class="control-label"><b>{{trans('Widgets::widgets.ads_script')}}:</b></label>
                                <div class="">
                                    ...
                                    @if ($errors->has('scripts'))
                                        <span class="help-block">
                                                    <strong class="text-danger">{{ $errors->first('scripts') }}</strong>
                                                </span>
                                    @endif
                                </div>
                            </div>
                            @if(isset($item->trans))
                                <div class="form-group">
                                    <label class="control-label"><b>{{trans('Widgets::widgets.current_ad')}}:</b></label>
                                    <div class="">
                                        {!! $item->trans->scripts !!}
                                    </div>
                                </div>
                            ...
                            @include('Core::fields.input_text', [
                                    'field_name' => 'start_at',
                                    'name' => trans('Widgets::widgets.start_at'),
                                    'placeholder' => trans('Widgets::widgets.start_at'),
                                    'value' => isset($item) ? old('start_at',$item->trans->start_at) : ""
                                ])
                            @include('Core::fields.input_text', [
                                    'field_name' => 'end_at',
                                    'name' => trans('Widgets::widgets.end_at'),
                                    'placeholder' => trans('Widgets::widgets.end_at'),
