    ...
    <div class="row">
        <div class="col-md-12">
            <div class="admin-top-operation">
                <a class="btn btn-default" href="{{ action('\UiStacks\Tutorials\Controllers\CoursesController@create')}}">{{ trans('Core::operations.create') }} {{ trans('Tutorials::tutorials.course') }}</a>
            </div>
            @if($items->count() || $_GET)
                                ...
                                @foreach($items as $item)
                                    @if(isset($item->trans))
                                        <tr @if($item->trans) data-title="{{ $item->trans->name }}" @endif>
                                            ...
                                            <td>
                                                <a href="{{ action('\UiStacks\Tutorials\Controllers\CoursesController@edit', $item->id) }}"><i class="fa fa-edit"></i> {{ trans('Core::operations.edit') }}</a>
                                                &nbsp;<a href="{{ action('\UiStacks\Tutorials\Controllers\SectionsController@index', $item->id) }}"><i class="fa fa-table"></i> Sections</a>&nbsp;
