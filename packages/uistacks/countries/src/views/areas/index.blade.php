...
@section('page_title')
{{ trans('Countries::countries.areas') }}
@endsection
@section('content')
...
<div class="row">
    <div class="col-md-12">
        ...
        <div class="admin-top-operation">
            <a class="btn btn-default" href="{{ action('\UiStacks\Countries\Controllers\AreasController@create')}}">{{ trans('Core::operations.create') }} {{ trans('Countries::countries.area') }}</a>
        </div>

        @if($items->count() || $_GET)
            @include('Countries::areas.filter')
        @endif

        @if($items->count())
            <form method="POST" action="{{ action('\UiStacks\Countries\Controllers\AreasController@bulkOperations')}}" id="bulk" class="form-inline">
                    ...
                    <table class="table table-striped table-hover">
                        <thead>
                            <tr>
                                ...
                                <th>{{ trans('Countries::countries.area') }}</th>
                            ...
                            @foreach($items as $item)
                                <tr @if($item->trans) data-title="{{ $item->trans->name }}" @endif>
                                    ...
                                    <td class="user_name_col_{{$item->id}}">
                                        @if($item->country && $item->country->trans)
                                    ...
                                    <td>
                                        ...
                                        @endif
                                    <td>
                                       <a href="{{ action('\UiStacks\Countries\Controllers\AreasController@edit', $item->id) }}"><i class="fa fa-edit"></i> {{ trans('Core::operations.edit') }}</a>
        ...
        @else
            <div class="text-center">
                @if($_GET)
                    <h1>{{ trans('admin.no_results_found') }} <a href="{{ action('\UiStacks\Countries\Controllers\AreasController@index')}}">{{ trans('admin.back') }}</a></h1>
                @elseif(Request::is(''.Lang::getlocale().'/admin/countries-areas'))
                    <h1>{{ trans('admin.no_data_added_before') }} <a href="{{ action('\UiStacks\Countries\Controllers\AreasController@create')}}">{{ trans('Core::operations.create') }} {{ trans('Countries::countries.area') }}</a></h1>
                @endif
             </div>
        @endif
    </div>
