...
<div class="row">
    <div class="col-md-12">
        ...
        <div class="admin-top-operation">
            <a class="btn btn-default" href="{{ action('\UiStacks\Countries\Controllers\CountriesController@create')}}">{{ trans('Core::operations.create') }} {{ trans('Countries::countries.country') }}</a>
        </div>

        @if($items->count() || $_GET)
            @include('Countries::countries.filter')
        @endif

        @if($items->count())
            <form method="POST" action="{{ action('\UiStacks\Countries\Controllers\CountriesController@bulkOperations')}}" id="bulk" class="form-inline">
                    ...
                    <table class="table table-striped table-hover">
                        <thead>
                            <tr>
                                ...
                                <th>{{ trans('Countries::countries.country') }}</th>
                            ...
                            @foreach($items as $item)
                                <tr @if($item->trans) data-title="{{ $item->trans->name }}" @endif>
                                    ...
                                    <td class="user_name_col_{{$item->id}}">
                                        ...
                                        @endphp
                                        <img src="{{isset($item->trans->flag) ? $exist_flag : ""}}" />
                                        @endif
                                        @if($item->trans)
                                    ...
                                    <td>
                                        ...
                                        @endif
                                    <td>
                                       <a href="{{ action('\UiStacks\Countries\Controllers\CountriesController@edit', $item->id) }}"><i class="fa fa-edit"></i> {{ trans('Core::operations.edit') }}</a>
        ...
        @else
            <div class="text-center">
                @if($_GET)
                    <h1>{{ trans('admin.no_results_found') }} <a href="{{ action('\UiStacks\Countries\Controllers\CountriesController@index')}}">{{ trans('admin.back') }}</a></h1>
                @elseif(Request::is(''.Lang::getlocale().'/admin/countries'))
                    <h1>{{ trans('admin.no_data_added_before') }} <a href="{{ action('\UiStacks\Countries\Controllers\CountriesController@create')}}">{{ trans('Core::operations.create') }} {{ trans('Countries::countries.country') }}</a></h1>
                @endif
             </div>
        @endif
    </div>
