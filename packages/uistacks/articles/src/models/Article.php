...
//    protected $appends = ['type', 'media'];
    protected $fillable = ['article_type', 'article_url','published'];
...
//    protected $with = ['trans', 'author', 'lastUpdateBy', 'itemSection'];
    ...
    {
//        return $this->hasOne('\UiStacks\Articles\Models\ArticleTrans', 'article_id')->where('lang', Lang::getlocale())->select(['article_id', 'title', 'body','lang','article_seo_title','article_meta_keywords','article_meta_descriptions']);
        return $this->hasOne('\UiStacks\Articles\Models\ArticleTrans', 'article_id')->where('lang', Lang::getlocale());
    }
	...
	{	
		if ($this->category == 1) {
			$category = trans('Articles::ads.ads');
		}elseif ($this->category == 2) {
			$category = trans('Articles::ads.services');
		}
	    return $category;
	}
    ...
    {
        return $this->belongsTo('uistacks\Articles\Models\Section', 'section');
    }
