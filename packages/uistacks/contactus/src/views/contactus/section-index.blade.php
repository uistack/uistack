@php
    $breadcrumbs = [
    ['url' => '', 'name' => trans('Contactus::contactus.contactus_section')]
    ];
...
{{--                @include('Contactus::contactus.filter')--}}
            @endif
            @if($items->count())
                <form method="POST" action="{{ action('\UiStacks\Contactus\Controllers\ContactusController@bulkOperationsSection')}}" id="bulk" class="form-inline">
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-hover">
                            ...
                            <tr>
                                ...
                                <th>{{ trans('Contactus::contactus.name') }}</th>
                                <th>{{ trans('Contactus::contactus.email') }}</th>
                            ...
                            @foreach($items as $item)
                                <tr @if($item->trans) data-title="{{ $item->trans->name }}" @endif>
                                    ...
                                    <td>
                                        <a href="{{ action('\UiStacks\Contactus\Controllers\ContactusController@editSection', $item->id) }}"><i class="fa fa-edit"></i> {{ trans('Core::operations.edit') }}</a>
            ...
            @else
                <div class="text-center">
                    ...
                    @elseif(Request::is(''.Lang::getlocale().'/admin/activities'))
                        <h1>{{ trans('admin.no_data_added_before') }} <a href="{{ action('\UiStacks\Activities\Controllers\ActivitiesController@create')}}">{{ trans('Core::operations.create') }} {{ trans('Contactus::contactus.activity') }}</a></h1>
                    @endif
                </div>
            @endif
        </div>
