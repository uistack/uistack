...
var mediaLibraryTab = '#inno_media_library_tab';
var uploadFilesTab = '#inno_upload_files_tab';

var containers = '.inno-container';
var mediaLibrary = '#inno_media_library';
var uploadFiles = '#inno_upload_files';

var maxUploadFileSize = 2000000;
var validateFileSizeErrorMsg = $('#max_file_size_validation').val();

var files;
var dropArea = document.getElementById("inno_upload_files_area");

var multipleSelect = false;
var individualRequired = false;

var selectButtonName;
var defaultSelectButtonName = 'اختر'
...
function openModalEvent() {
    $('#inno_media_modal').on('show.bs.modal', function(e) {
        // Reset to default settings
        $('#inno-no-more-files').hide();
        paginationPage = 1;
        isLoading = false;
        stopLoadPagination = false;
        dataPosition = 'top';
        // Load fresh data
        $('.inno-media-list ul li').remove();
        loadFiles();

        // Reset progress bar
        $('#inno_media_modal .upload-error').remove();
        ...
        $('#inno_media_modal .validation-error').remove();
        $('#inno_media_modal .upload-files-info').remove();
        $('#inno_media_modal .media-sidebar .upload-success-message').remove();
    });
...
function initMediafields() {
    ...
    $('[data-target="#inno_media_modal"]').click(function() {
        // Add overflow style to HTML
        $('html').css('overflow', 'hidden');
        // Remove other active media
        $('[data-target="#inno_media_modal"]').removeClass('active-media-modal');
        ...
        if (selectButtonName) {
            $('#inno_media_modal .modal-footer button#set-choosed-file').text(selectButtonName);
        } else {
            $('#inno_media_modal .modal-footer button#set-choosed-file').text(defaultSelectButtonName);
        }
...
function setMediafields() {
    ...
    var thumbnail;
    var mediaId;
    $('#inno_media_modal .modal-footer button#set-choosed-file').on('click', function() {
        // Check if media opend from richText or form anchor
        if (richTextModal == true) {
            $('#' + richTextFieldName).val($('.inno-media-list li.selected.details').children('div').children('img').attr('src').replace('thumbnail/', ''));
            richTextModal = false;
        } else {
            ...
            // Append items
            $('.inno-media-list li.selected').each(function() {
                ...
                // Check if gallery add items to parent .inno-media-gallery
                if (multipleSelect === true) {
                    $('.active-media-modal').parent().children('.inno-media-gallery').append(mediaItem);
                } else {
                    // Individual
...
function enableDisableSetChoosedButton() {
    // Default status
    $('#inno_media_modal .modal-footer button#set-choosed-file').prop("disabled", true);

    // Check if have any selected files.
    if ($('.inno-media-list li.selected').length > 0) {
        $('#inno_media_modal .modal-footer button#set-choosed-file').prop("disabled", false);
    } else if ($('.inno-media-list li.selected').length === 0) {
        // hide info at sidebar
        $('#inno_media_modal .file-info').hide();
    }
}
...
function styleBrowseButton() {
    $("#inno_browse_button").click(function() {
        $("#files").click();
    });
}
styleBrowseButton();
...
function drop() {
    dropArea.addEventListener("dragleave", function(evt) {
        ...
        if (target && target === dropArea) {
            this.className = "";
            $('.media-modal .inno-browse-form').css('display', 'block');
            $('.media-modal .inno-drag-info').css('display', 'none');
        }
        evt.preventDefault();
        evt.stopPropagation();
    }, false);
    ...
    dropArea.addEventListener("dragenter", function(evt) {
        ...
        $('.media-modal .inno-browse-form').css('display', 'none');
        $('.media-modal .inno-drag-info').css('display', 'block');
        evt.preventDefault();
        evt.stopPropagation();
    }, false);
 ...
 */
function init() {
    $('#inno_media_modal').on('change', '#files', prepareUpload);
}
init();
...
function prepareUpload(event) {
    ...
    if (notValidSizeFilesNames.length > 0) {
        ...
        $('#inno_upload_files_area').append(sizeValidationErrorMessage);
        $('#inno_media_modal .media-sidebar').prepend(sizeValidationErrorMessage);
    }

    // Upload valid files
    var uploadFilesMessage = {
        uploadProgress: 'جاري رفع ',
        fromOriginal: ' ملف من أصل ',
        fromSelected: ' ملف تم إختيارهم'
    };

    $('#inno_media_modal .progress').after('<div class="upload-files-info">' + uploadFilesMessage.uploadProgress + files.length + uploadFilesMessage.fromOriginal + selectedFiles.length + uploadFilesMessage.fromSelected + '</div>');
    if (files.length > 0) {
        ajaxUploadFiles();
    }
}
...
function ajaxUploadFiles() {
    ...
    $.ajax({
        ...
        xhr: function() {
            ...
            if (xhr.upload) {
                xhr.upload.addEventListener('progress', function(event) {
                    ...
                    //update progress bar
                    $('#inno_media_modal .upload-error').remove();
                    $("#inno_media_modal .progress").show();
                    $("#inno_media_modal .progress .progress-bar").css("width", +progressPercent + "%");
                    $("#inno_media_modal .progress .progress-bar").text(progressPercent + "%");
                    $("#inno_media_modal .progress .progress-bar").attr('aria-valuenow', progressPercent);
                    $("#inno_media_modal .progress .progress-bar").css("background-color", "#337ab7");
                }, true);
            }
            return xhr;
        },
        ...
        $("#inno_media_modal .progress .progress-bar").css("background-color", "#F44336");
        var uploadError = '<div class="upload-error alert alert-danger">' + error.responseJSON.message + '</div>';
        $('#inno_upload_files_area').append(uploadError);
    });
}
...
function uploadedSuccess(data, dataPosition, newData) {
    ...
    var successUploadMessage = 'ملف تم اضافته بنجاح';
    var successUpload = '';
    ...
    $('#inno_media_modal .media-sidebar').prepend(successUpload);
    // Active container
    activeContainer(mediaLibrary);
}
...
function addUploadedDataToMediaLibrary(data, dataPosition, newData) { // default top for new uploaded data
    ...
    if (dataPosition == 'bottom') {
        $('.inno-media-list ul').append(thumbnail);
    } else if (dataPosition == 'search') {
        $(".inno-media-list ul").replaceWith('<ul>' + thumbnail + '</ul>');
    } else {
        $('.inno-media-list ul').prepend(thumbnail);
        // Select new files
        if (newData === true) {
            data.reverse();
            for (var i = 0; i < data.length; i++) {
                activeFileDetails($('.inno-media-list ul li[data-id="' + data[i].id + '"] div'));
            }
        }
...
function searchFiles() {
    $('#inno_media_search').off('keyup').on('keyup', function() {
        // Empty
        $('.inno-media-list ul li').remove();
        $('#inno-no-more-files').hide();

        // load filss
        loadFiles($('#inno_media_search').val(), 1);
    });
}
searchFiles();

function scrollPaginationLoad() {
    $('.inno-media-list').off('scroll').on('scroll', function() {
        if ($('.inno-media-list').scrollTop() + $('.inno-media-list').height() >= $('.inno-media-list ul').height() && !isLoading && !stopLoadPagination) {
            isLoading = true;
            paginationPage++;
...
function toggleSelectFile() {
    ...
    $('.inno-media-list li button').off("click").on('click', function() {
    ...
    $('.inno-media-list li div').off("click").on('click', function() {
...
function activeFileDetails($selectedListItemDiv) {
    ...
    if (multipleSelect === false) {
        $('.inno-media-list li').removeClass('selected');
        $('.inno-media-list li button').removeClass('active');
    }
    $('.inno-media-list li').removeClass('details');
    ...
    $('#inno_media_modal .file-info .thumbnail img').attr('src', javascript_site_path + getThumbnailCoverImage(activeItem));
    $('#inno_media_modal .file-info .details .filename b').text(activeItem.filename);
    $('#inno_media_modal .file-info .details .uploaded').text(activeItem.created_at);
    $('#inno_media_modal .file-info .details .file-size').text(activeItem.file_size);
    $('#inno_media_modal .file-info .details .dimensions').text(activeItem.width + ' × ' + activeItem.height);
    ...
    $('#inno_media_modal .file-info').show();
...
function activeOtherSelectedFileDetails() {
    $('.inno-media-list li.selected div').each(function(index, element) {
...
function closeModalEvent() {
    $('#inno_media_modal').on('hide.bs.modal', function(e) {
        $('html').css('overflow', '');
        // Rest settings
